        ...
        <main>
            <div class="as-invoice invoice_style1">
                    ...
                    <header class="as-header header-layout1">
                        ...
                        <div class="header-bottom">
                            <div class="row align-items-center">
                                ...
                                <div class="col-auto"><p class="invoice-number me-4"><b>Cash Purchase Invoice</b></p>
                                </div>
                                <div class="col-auto"><p class="invoice-date"><b>Date: </b>{{date('Y-m-d')}}</p></div>
                            </div>
                        </div>
                    </header>
                    <div class="row justify-content-between mb-4">
                        <div class="col-auto">
                            @if ($purchase->purchase_type =='Debit')
                                <div class="invoice-left"><b>Supplier Details:</b>
                                    <address>Name:{{$purchase->name}}<br>Mobile:{{$purchase->mobile}}<br>
                                    </address>
                                </div>
                            @elseif($purchase->purchase_type =='Credit')
                                <div class="invoice-left"><b>Supplier Details:</b>
                                    <address>Name: {{$purchase->supplier->name}}<br>Mobile: {{$purchase->supplier->mobile}}<br>Address: {{$purchase->supplier->address}}
                                    </address>
                                </div>
                            @endif
                        </div>
                    ...
                    <table class="invoice-table">
                        ...
                        @foreach($purchase->details as $detail)
                            <tr>
                                ...
                                <td>{{dateFormat($detail->created_at,'Y-m-d')}}</td>
                                <td>{{$detail->product->name}}</td>
                                <td>{{$detail->quantity}}&nbsp{{$detail->unit_id}}</td>
                                <td>{{$detail->price}}</td>
                    ...
                    <div class="row justify-content-between">
                        <div class="col-auto">
                            <div class="invoice-left"><b>Terms & Conditions</b>
                                <p class="mb-0">Authoritatively envisioneer business<br>action items through parallel
                                    sources.</p></div>
                        </div>
                        <div class="col-auto">
                            <table class="total-table">
                                ...
                                @if($purchase->purchase_type =='Credit')
                                    ...
                                    <tr>
                                        <th>Payable:</th>
                                        <td>{{$purchase->total_amount-$purchase->pay_amount}}</td>
                                    </tr>
                                    <tr>
                                        @php($balance = supplierBalanceCal($purchase->supplier->id))
                                        <th>Past Payable:</th>
                                        <td>{{$balance['balance']}}</td>
                                    </tr>
                                    <tr>
                                        <th>Present Payable:</th>
                                        <td>{{$balance['balance']+$purchase->pay_amount}}</td>
                                    </tr>
                                @endif
                                <tr>
                                    <th>Labor Cost:</th>
                                    <td>{{$purchase->labor_cost}}</td>
                                </tr>
                                <tr>
                ...
                <div class="invoice-buttons">
                    ...
                    <a id="download_btn" class="download_btn"  href="{{route('cash.purchase.report.page')}}">
